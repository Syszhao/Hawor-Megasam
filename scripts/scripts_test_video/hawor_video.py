...
def hawor_infiller(args, start_idx, end_idx, frame_chunks_all):
    ...
    fpath = get_slam_path(seq_folder, start_idx, end_idx, get_slam_backend(args))
    ...
    world_space_payload = [pred_trans, pred_rot, pred_hand_pose, pred_betas, pred_valid]
    backend = get_slam_backend(args)
    backend_save_path = os.path.join(seq_folder, f"world_space_res_{backend}.pth")
    legacy_save_path = os.path.join(seq_folder, "world_space_res.pth")
    joblib.dump(world_space_payload, backend_save_path)
    joblib.dump(world_space_payload, legacy_save_path)
    print(f"Saved world-space result: {backend_save_path}")
